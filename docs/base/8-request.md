...
> `q-uniapp` 里面是使用 `service文件夹` 后面不再说明。
...
`script` 部分使用封装好的 `useRequest` 即可实现请求状态一体化，如下：

```ts
<script setup>
type IFooItem = { name: string }
const { loading, error, data, run } = useRequest<IFooItem>(() => httpGet('/foo', { name: 'uglyp' }))
</script>
```
...
const { loading, error, data, run } = useRequest<IFooItem>(() => getFooAPI('uglyp'))
</script>
```
...
`script` 部分使用封装好的 `useUpload` 即可实现请求状态一体化，如下：

```ts
<script lang="ts" setup>
const { loading, data, run } = useUpload<string>({ user: 'uglyp' })
</script>
```
